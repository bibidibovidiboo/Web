...
public class JSONManager {
   public List<MovieVO> movieListData(int no)
   {
      ...
      //실제 주소 http://www.kobis.or.kr/kobis/business/main/main.do
         try {
            String site="";
            if(no==1)
            {
            ...
            // [] => JSONArray
            // {} => JSONObject
            JSONArray arr =(JSONArray)jp.parse(json.text()); //parse를 하면 데이터가 object형으로 넘어오기때문에 JsonArray로 형변환
            ...
            for(int i=0;i<arr.size();i++)
            {
               ...
               //System.out.println("영 화 명:"+vo.getMovieNm());
               //System.out.println("영문 이름:"+vo.getMovieNmEn());
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         JSONParser jp= new JSONParser(); 
         JSONArray arr=(JSONArray)jp.parse(movie_data);//JSON은 OBJECT형이므로 형변환 해야한다. 이유는 [ ] 배열 형태로 들어와서
         System.out.println(arr.toJSONString());
         for(int i=0;i<arr.size();i++)
         {
            JSONObject obj=(JSONObject)arr.get(i);
