...
public class DataBoardDAO {
   ...
   public static DataBoardVO boardDetailData(int no)
   {
	   ...
	   SqlSession session = null;
	   // 데이터를 가지고 오는 과정에 에러가 발생할 수 있다 => 사전에 에러 방지 프로그램 => 예외처리
	   /*
	    * 	예외처리
	   ...
	   try {
		  ...
		  session=ssf.openSession(); // Commit을 포함 하지 않고 있다.
		  
		  //상세보기는
		  // 1. 조회수 증가
		  session.update("hitIncrement",no);
		  // 2. 증가된 조회수를 포함해서 데이터 가지고 오기 ( 증가 시킨후 그값도 가지고온다)
		  // <select id="boardDetailData" resultType="DataBoardVO" parameterType="int">
		  vo=session.selectOne("boardDetailData",no);
		  /*
		   * 	Commit => 저장된 데이터가 변경,추가,삭제
		   * 			SELECT : 검색 
		   */
		  /*
		   * 	<SELECT>
   ...
   public static int boardFindCount(Map map) 
   {
	   ...
	   SqlSession session = null;
	   try {
		   // 연결 객체 주소 가지고 오기 : DBCP (미리 오라클을 연결한 Connection을 미리 생성 => 생성된 주소를 얻어와야한다.
		   // default => Connection을 8개 생성
		   session = ssf.openSession();		
   ...
   public static List<DataBoardVO> boardFindData(Map map)
   {
	  ...
	  try {
		  //객체 주소 얻기
		  session=ssf.openSession(); //commit은 필요없다
