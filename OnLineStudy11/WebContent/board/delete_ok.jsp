...
<%-- delete_ok.jsp : 데이터를 받아서 처리만 하고 다른 jsp로 이동 (HTML을 사용 할 필요가 없다) --%>
<%
	// detail.jsp에서 데이터를 두개 보냄 (게시물 번호 , 비밀번호)
	// 한글이 없다 => request.setCharcterEncoding()
	...
	// DAO 연결 => 비밀번호가 같으면 (삭제) , 비밀번호가 틀리면 삭제하지 않는다
 	...
 	{
 		// 처리 => 업로드 된 파일을 삭제
 		if(vo.getFilesize()>0){ // filesize=0(업로드없는 게시물) filesize>0 (업로드 된 파일)
 			File dir=new File("c:\\download");
 			// File => 실제 file , 폴더 directory
 			File[] files=dir.listFiles(); // download 안에 있는 모든 파일정보 가져옴
 			for(File f:files){
